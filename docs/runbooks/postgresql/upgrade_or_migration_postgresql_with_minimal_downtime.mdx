...
This process is about 9 steps and takes a matter of minutes **not including your dump/restore time**.
...
```sql title="Create replication slot"
...
```shell title="Connect to psql on your _new database_"
...
```sql title="Old database current WAL LSN"
...
```sql title="New database most recent LSN"
...
### 8. Database promotion Cutover your application to the new database
...
Run the following command from the PG 15's shell or an instance that can access both databases:
